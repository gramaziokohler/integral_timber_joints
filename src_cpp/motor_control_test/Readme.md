...
Motor used is **555 with 1:51 gearbox** (instead of the 1:100 to avoid accidental overloading and breaking other parts). Encoder is 2244 steps/rev. 

Electronics is similar to previous test. A **trapezoidal motion profile** is used in the controller, with a non-aggressive acceleration = **2000 step per s^2** and a running speed of **2805 step/s** (1.25 rpm / 5mm/s @ 1204 screw).
...
Voltage supply to motor driver (**12V**) with capped current (**7A**) by the large Power Supply
...
The controller monitors the speed (discrete diff of encoder position), the controller stops the motor when speed drops below threshold, 

| Speed Threshold [rev/sec] | Speed Threshold [step/s] | Imaginary Linear Speed 1204 Screw [mm/s] |
| ------------------------- | ------------------------ | ---------------------------------------- |
| 1                         | 2244                     | 4                                        |
| 0.5                       | 1122                     | 2                                        |
| 0.1                       | 224.4                    | 0.4                                      |
...
We observe the maximum force reading in the entire process.

#### Result

Plotting pull force vs speed threshold

## Motor09b_StopCondition_PosError

Assuming a fixed error threshold of 100 steps.
